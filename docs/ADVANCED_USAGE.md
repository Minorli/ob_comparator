...
本手册聚焦当前版本最常用的高级能力：Remap 推导、授权生成、VIEW rewrite 边界、运行时降级、报告入库、run_fixup 高级执行，以及表数据风险校验。
...
- 口径说明：
  - `trigger_status_report.txt` / 主报告中的 `compare_missing_total` = compare 层总缺失 trigger
  - `selected_missing` = 被 `trigger_list` 命中的缺失 trigger
  - `filtered_missing` = 缺失 trigger 中未被清单选中的数量
  - `fixup_skip_summary_<ts>.txt` 的 `task_total` = 最终 runnable trigger fixup 数量，不等于 `selected_missing`
  - 若父 TABLE 已被 compare 判定为缺失/人工处理/不支持，对应 TRIGGER 会先从 fixup compare scope 中剔除；主报告、`fixup_skip_summary` 与 `report_db` 保持同一基础口径
...
- `grant_generation_mode`：
  - `full`：默认模式，尽量镜像 Oracle 当前授权（对象/列级/系统/角色 + 依赖补充）
...
说明：
- `structural` 不会改变对象 compare/fixup，只会收窄授权脚本输出口径。
- `structural` 刻意跳过列级授权、系统权限、角色授权和 `WITH GRANT OPTION`；这部分应由业务或 DBA 另行处理。
- `structural` 仍会复用 capability probe、缺失过滤、延后分类和不支持授权报告，因此 `grants_miss/` 只包含当前模式下“缺失且可执行”的授权。
- `grant_tab_privs_scope` 在 `structural` 下不影响最终授权脚本内容，程序只把它当作 Oracle 权限盘点范围。
...
- 同义词链路会下钻到最终目标对象生成授权
- VIEW DDL rewrite 只处理表类数据来源（`TABLE/VIEW/MVIEW/SYNONYM`）；`FUNCTION/PACKAGE/PROCEDURE/SEQUENCE/TYPE` 只保留诊断，不参与 rewrite。
- 同义词只有终点确认是表类对象时才会参与 VIEW rewrite；终点若是 callable，会保留原文并只输出诊断，不做盲目改写。
...
- unresolved VIEW 依赖会按 `ORACLE_SYSTEM / CROSS_SCHEMA_OK / CROSS_SCHEMA_MISSING / UNKNOWN` 分类；真正需要动作的跨 schema 缺失继续保留为 warning，其余降为 debug/诊断。
...
- 升级兼容：启动时会自动扩容 report_db 中过窄的 `STATUS/REASON/DETAIL` 列，尽量兼容旧版本建出的表结构。
- 写库表清单：
...
- `runtime_degraded_detail_<timestamp>.txt`：若存在，先判断本轮是否为 `compare incomplete`；`COMPARE` 级事件表示当前结论不是完整 compare，`ARTIFACT` 级事件仅影响依赖链附件等辅助产物。
...
- grant rewrite 采用唯一临时文件并在失败路径自动清理，避免遗留 `.tmp` 干扰后续执行
...
### 6.5 大图依赖与运行时降级
- `print_dependency_chains=true` 时，程序会优先输出 `dependency_chains_*.txt` 供审计；但大图场景下可能提前跳过或截断链路，避免附件导出拖慢主 compare。
- `JOB_ACTION` / scoped text fallback 在单个 job 命中超大程序单元时，会触发大文本跳过、高扇出跳过或递归封顶；这些都会写入 `runtime_degraded_detail_<ts>.txt`。
- 交付或验收时，若主报告出现 `compare incomplete`，应把该轮结果视为 partial compare，而不是最终基线。
...
- 语句级执行模式下，若某条语句超时，该脚本剩余语句会停止继续执行；普通失败仍保持逐语句继续，避免“前面超时、后面又偷偷跑了一段”。
- Oracle Q-quote 字面量中的单独一行 `/` 不再被误判为 PL/SQL 终止符。
...
更新时间：2026-05-07 (V0.9.9.6-hotfix5)
