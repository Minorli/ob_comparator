...
### 修复
- 补齐 0.9.9.6+ 的交付包约束与启动期缺文件提示：主程序、修复执行器和诊断包如果缺少同目录内的 `comparator_reliability.py`，现在会明确提示这是内部伴随文件，不是第三方 pip 依赖。
- 同步 README、现场版手册、配置说明和部署文档，明确客户应使用 toolkit zip 或整仓更新，不要只替换单个脚本。
