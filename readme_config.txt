...
- report_db_save_full_json：是否保存完整报告 JSON。默认：false。
  ...
  - core: summary + DIFF_REPORT_DETAIL / DIFF_REPORT_GRANT / DIFF_REPORT_USABILITY / DIFF_REPORT_PACKAGE_COMPARE / DIFF_REPORT_TRIGGER_STATUS / DIFF_REPORT_EXCLUDED_OBJECT
  - full: core + DIFF_REPORT_DETAIL_ITEM / DIFF_REPORT_ARTIFACT / DIFF_REPORT_ARTIFACT_LINE / DIFF_REPORT_DEPENDENCY / DIFF_REPORT_VIEW_CHAIN / DIFF_REPORT_REMAP_CONFLICT / DIFF_REPORT_OBJECT_MAPPING / DIFF_REPORT_BLACKLIST / DIFF_REPORT_EXCLUDED_OBJECT / DIFF_REPORT_FIXUP_SKIP / DIFF_REPORT_OMS_MISSING
...
- extra_constraint_cleanup_mode：额外约束可执行清理模式。默认：safe_only。
  可选值：off、safe_only、semantic_fk_check。
  说明：`safe_only` 仅生成重复单列 `IS NOT NULL` 的安全清理 SQL；`semantic_fk_check` 会在此基础上，为 compare 后仍判定为 target-only 的 `FK/CHECK` 额外生成 `fixup_scripts/cleanup_semantic/constraint/*.sql`；该目录默认也会被 run_fixup 跳过，需显式 `--only-dirs cleanup_semantic/constraint`。
