...
def get_object_parent_tables(ora_cfg: OraConfig, schemas_list: List[str]) -> ObjectParentMap:
    ...
    try:
        with oracledb.connect(
            ...
            log.info("已获取 %d 个依附对象的父表映射（用于 one-to-many schema 拆分）。", len(parent_map))
    except oracledb.Error as e:
