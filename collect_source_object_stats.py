...
def fetch_object_counts(
    ...
    if "CONSTRAINT" in {obj.upper() for obj in object_types}:
        sql_tpl = """
            SELECT OWNER, COUNT(*)
            FROM DBA_CONSTRAINTS
            WHERE OWNER IN ({owners})
        ...
        with conn.cursor() as cursor:
            for chunk in chunk_list(owners, ORACLE_IN_BATCH_SIZE):
                ...
                for owner, count in cursor.fetchall():
                    ...
                    if not owner_u:
                        continue
                    results[owner_u]["CONSTRAINT"] += int(count or 0)
...
def bounded_percentile_value(counts: List[int], percentile: float) -> int:
    if not counts:
        return 0
    sorted_counts = sorted(counts)
    if len(sorted_counts) == 1:
        return sorted_counts[0]
    index = int(len(sorted_counts) * percentile)
    index = max(0, min(len(sorted_counts) - 1, index))
...
def summarize_table_stats(label: str, data: Dict[str, int], top_n: int) -> None:
    ...
    if not data:
        print("no data")
        return
    counts = sorted(data.values())
    total = sum(counts)
    table_count = len(counts)
    avg = total / table_count if table_count else 0
    p95 = bounded_percentile_value(counts, 0.95)
    p99 = bounded_percentile_value(counts, 0.99)
