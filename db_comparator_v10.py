...
4.  验证 remap_rules 中的源对象是否在源库中真实存在。
5.  生成 "主校验清单"，并检测 "多对一" 映射。
6.  使用 obclient CLI 校验 OceanBase 目标端：
    - VIEW: 检查是否存在。
    - TABLE: 检查是否存在，并对比列名集合。
7.  对比列名时，自动忽略 (过滤掉) 目标端以 'OMS_' 开头的列。
8.  输出带有颜色、无图标的、易于理解的最终报告。

新增功能 (V10 - 不破坏原有逻辑):
9.  基于已映射的 TABLE，进一步对比：
    - 索引 (INDEX)：名称、唯一性、列集合。
    - 约束 (CONSTRAINT)：主键/唯一键/外键 (P/U/R) 的名称、类型、列集合。
    - 表级触发器 (TRIGGER)：名称、触发事件、状态 (简单校验)。
10. 按 schema 对比序列 (SEQUENCE)：
    - 基于 schema 映射 (如果同一源 schema 只映射到唯一目标 schema，则使用该映射)，否则默认同名 schema 1:1。
    - 简化为存在性对比 (名称集合对比)。

注意:
- 原有 TABLE/VIEW 校验报告结构保持不变，新增加的对象在报告中以附加章节呈现。
- 新增对象不要求都在 remap_rules 中定义；索引/约束/触发器通过表映射关系推导，序列按 schema 映射推导。
"""
...
def print_final_report(
    ...
    print(f"           {Color.BOLD}数据库对象迁移校验报告 (V10){Color.ENDC}")
