...
- Oracle -> OB 链路可显式启用同一套定义级差异 compare：`synonym_definition_compare=true`、`partition_definition_compare=true` 后进入 SYNONYM/分区明细报告；可运行的本地同义词漂移可配合 `synonym_definition_fixup=true` 输出安全 fixup，且继续遵守 `synonym_fixup_scope`，分区定义漂移保持 report-only。
