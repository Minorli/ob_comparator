...
**验证记录 (2026-02-03)**:
在 Oracle 实库创建视图并生成 DDL，执行 remap 后确认别名未被替换。

原始 DDL 片段：
```sql
CREATE OR REPLACE FORCE EDITIONABLE VIEW "OMS_USER"."CMP_V_ALIAS_110801" ("ID") AS
SELECT CMP_ALIAS_110801.ID FROM OMS_USER.CMP_BASE_110801 CMP_ALIAS_110801
```

Remap 后片段：
```sql
CREATE OR REPLACE FORCE EDITIONABLE VIEW "OMS_USER"."CMP_V_ALIAS_110801" ("ID") AS
SELECT CMP_ALIAS_110801.ID FROM ZZ_APP.CMP_BASE_110801 CMP_ALIAS_110801
```

结论：别名 `CMP_ALIAS_110801` 未被替换为 `ZZ_APP.CMP_ALIAS_110801`，视图别名误替换未复现。
