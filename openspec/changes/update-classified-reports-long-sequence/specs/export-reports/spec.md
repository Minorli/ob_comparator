...
- **THEN** the file starts with a `#` header line and uses `|`-delimited columns

### Requirement: Root-cause tracing in blocked detail
The system SHALL include a ROOT_CAUSE column in per-type unsupported/blocked detail exports.

#### Scenario: Blocked object has root cause
- **WHEN** an object is blocked by unsupported dependencies
- **THEN** the per-type unsupported report includes ROOT_CAUSE with the ultimate unsupported object
