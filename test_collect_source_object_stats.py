...
class TestCollectSourceObjectStats(unittest.TestCase):
    def test_constraint_stats_template_includes_check_constraints(self):
        self.assertIn("'C'", cs.CONSTRAINT_TABLE_STATS_SQL)
    ...
    def test_bounded_percentile_value_handles_small_inputs(self):
        self.assertEqual(cs.bounded_percentile_value([], 0.95), 0)
        self.assertEqual(cs.bounded_percentile_value([7], 0.95), 7)
        self.assertEqual(cs.bounded_percentile_value([1, 2, 3, 4], 0.95), 4)
